...
const Avatar = ({ name, dp, size = "w-8 h-8" }) => {
...
const LocationBubble = ({ lat, lng }) => {
  ...
  return (
    <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="block rounded-xl overflow-hidden border border-white/20 hover:border-violet-300 transition-colors">
      <img src={imgUrl} alt="Location" className="w-56 h-28 object-cover" onError={(e) => { e.target.style.display = "none"; }} />
...
const ReactionsBar = ({ reactions, onToggle }) => {
...
const DeliveryTick = ({ msg, deliveryStatus }) => {
  const status = deliveryStatus?.[msg.clientMessageId];
...
const MessageBubble = ({ msg, isOwn, onEdit, onDelete, onReply, onPin, onReact, deliveryStatus }) => {
      ...
      <div className={`relative max-w-[70%] flex flex-col ${isOwn ? "items-end" : "items-start"}`}>
        ...
        <div className="relative">
          <ReactionPicker
            ...
            onSelect={(emoji) => { onReact(msg._id, emoji); setShowReactionPicker(false); }}
          />
        </div>
