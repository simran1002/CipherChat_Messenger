...
/**
 * Consumer group {@code notifications}. Each handler is one transaction:
 * ledger claim + row insert commit together, the offset is committed after
 * the method returns. Crash anywhere → redelivery → claim loses → no-op.
 */
@Component
...
public class NotificationConsumer {
    ...
    public NotificationConsumer(NotificationRepository notifications, ProcessedEventLedger ledger) {
    ...
    public void on(MessageSent e) {
        if (e.mentions().isEmpty() || !ledger.claim(CONSUMER, e.eventId())) return;
