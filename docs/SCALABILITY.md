...
Java gateway, one pod, measured 2026-09-05/06 ([BENCHMARKS.md](BENCHMARKS.md)): send → ACK p95 54 ms and broadcast p95 64 ms at 43 msg/s; 5,000 STOMP sockets held with zero failures at ≈ 97 KB heap each; a broadcast to all 5,000 subscribers of one room reaches every socket at p50 1.0 s / p95 6.6 s (serial delivery on the simple broker, ≈ 1.3 ms per subscriber), which is the concrete argument for sharded delivery above a few thousand members per room. 10,000 sockets did not complete on the shared Docker Desktop VM. Previous (Node) implementation, single node: 10,000/10,000 concurrent WebSocket connections held with heartbeat, roster broadcaster fix required to get there (the naive per-connect broadcast was O(N²)); the Java gateway keeps the same throttled roster design.
